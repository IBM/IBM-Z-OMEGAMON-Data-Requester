...
import { FalconDatasourceConfig_V6 } from './FalconDatasourceConfig_V6';

const FALCON_DATASOURCE_OPTIONS_VERSION_V7 = 7;

export function updateTo_V7(current: FalconDatasourceConfig_V6): FalconDatasourceConfig_V7 {
  const { falconVersion, ...newJsonData } = current.jsonData;
  return {
    ...current,
    jsonData: {
      ...newJsonData,
      adminUrl: '',
      falconVersion: FALCON_DATASOURCE_OPTIONS_VERSION_V7,
    },
  };
}
...
type GrafanaDefineJsonData_V7 = DEP_V7.DataSourceJsonData & {
  tlsSkipVerify: boolean;
  oauthPassThru: boolean;
  timeout: number; // http request timeout
};

type FalconDatasourceJsonData_V7 = GrafanaDefineJsonData_V7 &
  DEP_V7.WithVersion<typeof FALCON_DATASOURCE_OPTIONS_VERSION_V7> & {
    metadataCacheTimeInSec: number;
    adminUrl: string;
  };

export type FalconDatasourceConfig_V7 = DEP_V7.DataSourceSettings<FalconDatasourceJsonData_V7>;

/* Module without 'declare' keyword cannot have export declarations */
declare namespace DEP_V7 {
