...
export function updateTo_V6(current: FalconDatasourceConfig_V5): FalconDatasourceConfig_V6 {
  const { falconVersion, itmCacheTimeoutInSec, ...newJsonData } = current.jsonData;
  return {
    ...current,
    jsonData: {
      ...newJsonData,
      oauthPassThru: true,
      falconVersion: FALCON_DATASOURCE_OPTIONS_VERSION_V6,
    },
  };
}
...
type GrafanaDefineJsonData_V6 = DEP_V6.DataSourceJsonData & {
  tlsSkipVerify: boolean;
  oauthPassThru: boolean;
  timeout: number; // http request timeout
};

type FalconDatasourceJsonData_V6 = GrafanaDefineJsonData_V6 &
  DEP_V6.WithVersion<typeof FALCON_DATASOURCE_OPTIONS_VERSION_V6> & {
